...
rodent_list = [mouse.Mouse, rat.Rat, chinchilla.Chinchilla, capybara.Capybara, mole.Mole, porcupine.Porcupine, squirrel.Squirrel]
rodent_names = [x.__name__.lower() for x in rodent_list]

def pick_rodent():
    name = input('What is your name? ')
    while not name.isalpha() and len(name) < 2:
        name = input('Invalid input. (Only letters, and must be length > 1) What is your name? ')
    
    h_dict = {}
    for cl in rodent_list:
        h_dict[cl.__name__.lower()] = cl.__doc__, -1
    c = select('What type of rodent would you like to be?', *rodent_names, index=True, info_full = h_dict)
...
def random_death(rod):
    deaths = [' is defeated, blood pooling around it.', ' flies across the room, lifeless.', ' collapses, eyes rolling back.', ' is pummeled into the floor.', ' goes limp, head smacking the concrete.']
...
def start_combat(player, enemy):
    ...
    if len(fighter.friends) > 0:
        a = select('Would you like to fight by yourself, or tap a friend in?', 'self', 'friend')
    else:
        a = 'self'
    if a == 'friend':
        h_dict = {}
        ...
        f = select('Which friend?', *fighter.get_friends(), info_full=h_dict, index=True)
    ...
    if fight(fighter, enemy) == fighter:
        ...
        if fighter == player:
            ...
            if player.money < 10:
                print('You wake up, missing all ' + str(player.money) + ' of your scraps.')
                player.money = 0
                wait()
            else:
...
def cave(player):    
    ...
    clear()
    c = select('What would you like to do now?', 'fight', 'shady roach', 'quit')
    if c == 'fight':
        print('You decide you want to get into the ring once again.')
        if len(r_list) == 0:
            r_list = [random_enemy(player.level), random_enemy(player.level + 1), random_enemy(player.level + 2)]
        elif len(r_list) < 3:
            r_list.append(random_enemy(pre_lvl))
        print('You see three opponents of varying difficulty, your reward will be based on their level. Use info to learn their stats. ' + ' | '.join([str(i) for i in r_list]))
        h_dict = {}
        for r in r_list:
        ...
        if len(player.friends) > 0:
            ...
            c = select('Who would you like to fight?', *[i.name for i in r_list], 'friend', info_full=h_dict, index=True)
        else:
            c = select('Who would you like to fight?', *[i.name for i in r_list], info_full=h_dict, index=True)
        clear()
        if c == 3:
            h_dict = {}
            for i in player.friends:
                h_dict[i.name] = repr(i), -1
            f = select('Which friend?', *player.get_friends(), info_full=h_dict, index=True)
            c = player.friends[f]
            c.ai = True
            clear()
            del(player.friends[f])
...
if __name__ == '__main__':
    ...
    if skip:
        ...
        if fight(player, mouse.Mouse(name(), 40, 40, 40, 40, 40, ai=True)) == player:
            ...
            print('Russ: Wow, that\'s pretty embarassing. Well, it\'s okay. You will learn.')
        ...
        player.money += 10
        print('He hands you 10 random trinkets covered in dirt. Balance: ' + str(player.money))
        ...
        ShadyRoach.intro()
        clear()
        print('You exit his shop, overwhelmed by possibility, heading back to the main cave.')
    cave(player)
